...
const walkinPassType = ref("daily"); // 'daily' | 'weekly' | 'monthly'
...
async function checkInWalkIn() {
  ...
  try {
    const result = await $fetch("/api/checkins", { method: "POST", body: { name: walkinName.value, studentType: walkinStudentType.value, passType: walkinPassType.value } });
    feedback.value = { ok: true, text: `Walk-in "${walkinName.value}" (${walkinStudentType.value}${walkinPassType.value !== "daily" ? `, ${walkinPassType.value} pass` : ""}) logged.${feeText(result)}` };
    walkinName.value = "";
    walkinPassType.value = "daily";
    await refresh();
    openServicesModal(result.id, result.name);
  } catch (e) {
    feedback.value = { ok: false, text: e.data?.statusMessage || "Check-in failed." };
    ...
    <div class="rs-mobile-stack" style="display: grid; grid-template-columns: 1.2fr 1fr; gap: 18px;">
      ...
      <div class="rs-card">
        ...
        <div v-for="c in pagedLog" :key="c.id" style="display:flex; align-items:center; gap:8px; padding:9px 0; border-bottom:1px solid #1c2026;"
          ...
          <span v-else-if="c.type === 'walkin'" style="font-size:10px; color:#aab0bb; border:1px solid #2a2f38; border-radius:4px; padding:2px 5px;">Walk-in</span>
