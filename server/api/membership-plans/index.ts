import MembershipPlan, { MEMBERSHIP_TIERS, DURATIONS, TIER_DURATIONS } from "../../utils/models/MembershipPlan";
...
export default defineEventHandler(async (event) => {
  ...
  if (method === "GET") {
    ...
    return plans.map((p) => ({ id: String(p._id), tier: p.tier, duration: p.duration, price: p.price, visitFee: p.visitFee }));
  }
  ...
  if (method === "POST") {
    ...
    const existing = await MembershipPlan.findOne({ tier: body.tier, duration: body.duration });
    if (existing) {
      throw createError({ statusCode: 409, statusMessage: "A plan for this tier and duration already exists." });
    }
    ...
    await logAudit(user, `Added membership plan: ${body.tier} / ${body.duration}`);
