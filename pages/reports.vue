<script setup>
const view = ref("goers"); // 'goers' | 'inventory'
...
<template>
  <div>
    ...
    <p style="font-size: 13.5px; color: #8a909b; margin: 6px 0 18px;">Gym attendance and inventory sales, viewed separately, broken down by day, week, or month.</p>
    ...
    <div style="display: flex; gap: 6px; align-items: center; margin-bottom: 22px;">
      <button
        v-for="p in periods"
        :key="p.id"
        class="rs-btn-secondary"
      ...
      <button class="rs-btn-secondary" @click="view === 'goers' ? exportGoers() : exportInventory()">Export CSV</button>
    </div>
